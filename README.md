...
| 📋 **ناوبری منو فقط با کیبورد (اجباری)** | ابزار `menu` منوی نوار منو و زیرمنوها را بدون ماوس باز و انتخاب می‌کند (Alt+کلید مخفف، type-ahead و کلیدهای جهتی: `Right` برای باز شدن زیرمنو و `Enter` برای انتخاب) — چون حرکت نشانگر روی منوی باز، زیرمنوها را می‌بندد. به‌علاوه، **تا منویی باز باشد، همهٔ عمل‌های ماوس توسط خود برنامه بلوک می‌شوند** و ایجنت با پیامی که آیتم‌های باز هم در آن فهرست‌اند، به کیبورد هدایت می‌شود؛ پس منو دیگر «خودبه‌خود» بسته نمی‌شود. |
...
| ✅ **صحت‌سنجی قبل از اتمام کار** | پیش از پذیرش `task_complete` موفق، ایجنت یک اسکرین‌شات تازه می‌گیرد و از مدل می‌خواهد نتیجه را با درخواست اصلی مقایسه کند (یک دور): اگر چیزی جا مانده باشد، کار را ادامه می‌دهد وگرنه با همان خلاصه تأیید می‌کند. گزارش صادقانهٔ شکست (`success=false`) دوباره بازرسی نمی‌شود. قابل خاموش‌کردن با `verify_on_completion`. |
| 🧩 **مهارت‌های قابل‌نصب (Skills)** | با گذاشتن یک فایل `.md` / `.txt` که یک رویهٔ تخصصی را شرح می‌دهد (مثلاً «چطور در این نرم‌افزار اکسپورت بگیرم») در پوشهٔ `skills`، آن مهارت خودکار به system prompt اضافه می‌شود؛ هیچ تغییری در کد لازم نیست. پوشه‌های `skills` در کنار پروژه و در `%APPDATA%\WinAgent\skills` خوانده می‌شوند. |
...
| 📝 **لاگ کامل ریکوست/ریسپانس + بستهٔ عیب‌یابی** | هر کار یک trace JSONL از گفتگوی کامل با مدل (پاسخ‌ها، تاخیر، توکن، خطاها — بدون کلید API و بدون اسکرین‌شات) می‌نویسد؛ با یک کلیک در Settings، همهٔ لاگ‌ها در یک zip قابل‌ارسال جمع می‌شوند. جزئیات در [«لاگینگ و ارسال گزارش خطا»](#لاگینگ-و-ارسال-گزارش-خطا--logging--bug-reports) |\n| 🪟 **مدیریت برنامه‌ها و پنجره‌ها** | اجرای برنامه با نام (Notepad, Chrome, Excel, Settings…)، مسیر، منوی استارت یا اپ‌های UWP؛ فوکوس/کوچک/بزرگ/بستن/جابه‌جایی پنجره‌ها؛ فهرست کنترل‌های Win32. |
...
| `verify_on_completion` | قبل از پذیرش پایان موفق کار، یک دور بازرسی نتیجه با اسکرین‌شات تازه (پیش‌فرض روشن). |
...
- هر تلاش اصلاح از دوم به بعد **متفاوت** از قبلی است تا مدلِ قطعی در چرخهٔ «همان خروجی خراب را دوباره» گیر نکند: پیام ترمیم پلکانی قوی‌تر می‌شود (در تلاش دوم دستور «فقط خروجی معتبر و مینیمال، بدون توصیف صفحه» و از تلاش سوم نمونهٔ دقیق شکل پاسخِ پروتکلِ فعال) و دمای تولید (temperature) در هر تلاش کمی بالا می‌رود (تا سقف ۱٫۵).
...
- **استثنا (پایان آرام):** اگر کار با ابزارها شروع شده باشد و مدل در پروتکل JSON، به‌جای اقدام، با **متن صادقانه** (متن ساده یا `{"message":"..."}`) پاسخ بدهد — حالتی که معمولاً یعنی «این مرحله ممکن نیست، رها می‌کنم» (مثلاً کنترل غیرفعال) — پس از تمام‌شدن تلاش‌ها، پاسخ متنی خودِ مدل به‌عنوان جواب نهایی نمایش داده می‌شود و خطای پروتکل نمی‌دهد. این فقط برای متنِ واقعی است؛ JSON ناقص/بریده‌شده همچنان خطا می‌دهد. درخواست اصلاح هم در پروتکل JSON، قالب دقیق `task_complete` را به مدل نشان می‌دهد.
...
### صحت‌سنجی قبل از اتمام کار / Completion verification

پیش از پذیرش پایان **موفق** یک کار (وقتی `task_complete` با `success=true` آمده باشد)، ایجنت یک اسکرین‌شات تازه می‌گیرد و از مدل می‌خواهد نتیجه را با **درخواست اصلی** مقایسه کند. این یک دور بازرسی است:

- اگر چیزی جا مانده یا اشتباه انجام شده باشد، مدل **نپذیرفتن** کار را ادامه می‌دهد و با اقدام بعدی درستش می‌کند.
- اگر واقعاً کامل شده باشد، با `task_complete` و همان خلاصه تأیید می‌کند.
- فقط **یک بار** برای هر کار انجام می‌شود (تلاش مجددِ بی‌پایان ندارد) و با سقف `max_steps` محدود است.
- گزارش **صادقانهٔ شکست** (`success=false`) و پاسخ‌های چتِ بدون ابزار دوباره بازرسی نمی‌شوند.
- هزینه: یک تماس و یک اسکرین‌شات اضافه برای هر کار موفق. برای خاموش‌کردن از **Settings → Agent → Verify the result before accepting task completion** یا `"verify_on_completion": false`.

### لاگینگ و ارسال گزارش خطا / Logging & bug reports

برنامه دو دستهٔ لاگ می‌نویسد — هر دو در `%APPDATA%\WinAgent\logs` (در سیستم‌های غیرویندوزی `~/.config/winagent/logs`؛ با متغیر محیطی `WINAGENT_LOGS_DIR` قابل جابجایی):

1. **`winagent.log`** — لاگ عمومی برنامه (سطح‌های DEBUG/INFO/…) با روتیشن ۵ مگابایت × ۳ نسخه. هم در GUI و هم در CLI نوشته می‌شود.
2. **`sessions/<تاریخ-ساعت>-<نام‌وظیفه>.jsonl`** — یک فایل برای **هر کار** که دقیقاً همان گفتگوی ایجنت با مدل است:
   - `session_start`: متن کامل وظیفه، نسخهٔ برنامه، مدل، پروتکل، تنظیمات (بدون کلید API) و فهرست مهارت‌ها
   - `llm_request`: هر درخواست به مدل — پیام‌های کامل (اسکرین‌شات‌ها فقط به‌صورت placeholder)، اسکیما ابزارها، دما، شمارهٔ retry
   - `llm_response`: هر پاسخ — `finish_reason`، تاخیر (ms)، توکن‌ها، پیامِ خودِ مدل و پاکت خام ارائه‌دهنده (`raw`)؛ در خطا، کد HTTP و متن خطای سرور
   - `llm_rejected`: پاسخ‌هایی که ارسالشان موفق بوده ولی قالب/محتوایشان رد شده، همراه با دلیل رد
   - `tool_results`: نتیجهٔ هر فراخوانی ابزار
   - `session_end`: وضعیت نهایی، تعداد گام/اقدام، زمان و مصرف توکن

**حریم خصوصی:** کلید API از همهٔ ریکورد‌ها پاک می‌شود (و در bundle فقط علامت `<set>`/`<unset>` ثبت می‌شود)، مقادیر فیلدهای `api_key`/`Authorization`/… ماسک می‌شوند و base64 اسکرین‌شات‌ها جایگزین placeholder می‌شوند. هر فایل session سقف ۱۰ مگابایت دارد و شکست در نوشتن لاگ هرگز کار را مختل نمی‌کند.

**ارسال برای عیب‌یابی:** **Settings → Safety → `Export log bundle…`** — یک فایل zip واحد (`WinAgent-log-bundle-….zip`) می‌سازد شامل `meta.json` (نسخه، پلتفرم، پیتون، تنظیمات بدون کلید)، `winagent.log` و تازه‌ترین traceهای جلسات. همین یک فایل را به‌همراه شرح کوتاهی از آنچه درست نبوده بفرستید. هنگام خطا یا رسیدن به سقف گام، مسیر همان trace جلسه هم در چت نمایش داده می‌شود.

گزینه‌ها: `log_level` (`INFO` پیش‌فرض؛ `DEBUG` برای جزئیات بیشتر) و `log_llm_trace` (خاموش‌کردن ثبت traceهای جلسه؛ پیش‌فرض روشن).

### مهارت‌ها / Skills (نصب رویه‌های تخصصی)

می‌توانید به ایجنت **مهارت‌های خاص** بیاموزید: هر مهارت یک فایل متنی (`.md` / `.txt`) است که یک رویهٔ مرحله‌به‌مرحله را شرح می‌دهد.

**از رابط کاربری:** **Settings → Agent → Install skill…** — پنجرهٔ ساخت/ویرایش/حذف مهارت (نام، متن، انتخاب پوشهٔ مقصد، بارگذاری فایل موجود، بازکردن پوشهٔ مهارت‌ها). مهارتِ تازه از **درخواست بعدی** بلافاصله مؤثر است — بدون ری‌استارت و بدون تغییر کد.

یا مستقیم فایل را در یکی از این دو پوشه بگذارید تا **خودکار** بارگذاری و به system prompt افزوده شود:

1. `skills/` در کنار پوشهٔ WinAgent (مشترک روی همان پروژه)
2. `%APPDATA%\WinAgent\skills` روی ویندوز (یا `~/.config/winagent/skills` در دیگر سیستم‌ها) — اختصاصی به هر کاربر

قوانین:

- فقط فایل‌های `.md`، `.markdown` و `.txt` خوانده می‌شوند؛ فایل‌هایی که نامشان با `_` یا `.` شروع می‌شود نادیده گرفته می‌شوند (برای نمونه/الگو). نمونهٔ آماده در `skills/_example.md`.
- هر مهارت تا ۴۰۰ نویسه و کل بخش تا ۱۶۰۰ نویسه نگه داشته می‌شود تا context پُر نشود؛ هرچیز بلندتر به‌صورت امن کوتاه می‌شود.
- اگر دو مهارت هم‌نام باشند، نسخهٔ پوشهٔ کاربری بر پروژه اولویت دارد.
- در **Settings → Agent** فهرست مهارت‌های نصب‌شده (با مسیرشان در tooltip) را می‌بینید.

نمونهٔ یک مهارت (`skills/invoice-export.md`):

```markdown
### وقتی استفاده شود
کاربر می‌خواهد از CRM فاکتور اکسپورت کند.

### مراحل
1. open_app "CRM" (صبر کن تا پنجره "Invoices – Acme CRM" آمد).
2. ابزار `menu` با path ["Report","Invoices","Export…"] (فقط کیبورد).
3. در دایالوگ: "PDF"، پوشه "C:\Reports\Invoices"، سپس Save.
4. روی اسکرین‌شات تأیید کن که خط "Export finished" دیده می‌شود.

### نکات
- آیتم "Export" تا یک بازهٔ تاریخ انتخاب نشود خاکستری است.
- اگر دایالوگ "license expired" آمد، چیزی را نکش؛ در task_complete با success=false گزارش کن.
```

ایجنت این رویه را هر جا که درخواست کاربر با مهارت مطابقت کند دنبال می‌کند؛ اگر مهارت و درخواست صریحِ فعلی کاربر در تضاد باشند، **درخواست فعلی کاربر** اولویت دارد.
...
├── skills.py           بارگذاری مهارت‌های قابل‌نصب (فایل‌های .md/.txt پوشهٔ skills) و رندرشان در system prompt
...
├── logging_setup.py    لاگ روتیشن‌شده + traceٔ JSONL ریکوست/ریسپانس هر کار (با پاک‌سازی کلید/تصویر)
├── logbundle.py        ساخت zip یک‌فایلیِ لاگ‌ها برای ارسال به توسعه‌دهنده
├── tools/
