...
| 🧪 **قابل تست بدون ویندوز** | یک بک‌اند شبیه‌سازی‌شده (`--demo`) امکان اجرای GUI و تست کامل حلقهٔ ایجنت را روی هر سیستم‌عاملی می‌دهد؛ ۱۳۰ تست خودکار همراه با یک سرور OpenAI ساختگی. |
...
python -m pytest            # 130 tests (protocol, executor, launcher, screen guard, agent loop, HTTP end-to-end, headless GUI)
