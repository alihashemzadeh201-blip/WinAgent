...
- پاسخ متنی عادی در حالت `native` معتبر است؛ در حالت `json` جواب متنی باید داخل `{"message":"..."}` باشد. اگر پاسخ‌ها مرتب بریده می‌شوند، مقدار **Max tokens** یا مدل انتخاب‌شده را بررسی کنید.
...
اگر ویندوز ماوس را به نقطهٔ دیگری ببرد یا محدود کند، تطابق موقعیت واقعی قبل از کلیک بررسی می‌شود و در صورت مغایرت کلیک ارسال نمی‌شود. در لاگ سطح `INFO` (پیش‌فرض)، سطرهای `Captured frame`، `Pointer mapping` و `Pointer after action` ابعاد تصویر، شناسهٔ فریم، نقطهٔ انتخابی مدل، هدف فیزیکی و موقعیت واقعی ماوس بعد از عمل را نشان می‌دهند. برای آزمایش بدون کلیک از `mouse_move` استفاده کنید. انتخاب اشتباه خودِ نقطه توسط مدل همچنان ممکن است؛ در این حالت تصویر تازه بگیرید و مرکز هدف را دوباره مشخص کنید.
