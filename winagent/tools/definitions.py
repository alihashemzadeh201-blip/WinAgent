...
TOOLS: list[ToolSpec] = [
    ...
    ToolSpec(
        ...
        description="Press several key combinations one after another, e.g. ['win+r'] then 'notepad' is better done with open_app; use this for menu navigation like ['alt', 'f', 's'].",
        parameters=_obj({
    ...
    ToolSpec(
        name="open_app",
        description=(
            "Launch an application by name (e.g. 'notepad', 'calculator', 'chrome', 'excel', 'settings'), by executable "
            "path, or open a document/folder path with its default program. Prefer this over clicking through the Start menu."
        ),
        parameters=_obj({
            "name": {"type": "string", "description": "Application name, executable, Start-menu entry, file or folder path."},
            "args": {"type": "string", "description": "Optional command line arguments."},
            "wait": {"type": "number", "description": "Seconds to wait for the app to appear before the screenshot (default 2)."},
