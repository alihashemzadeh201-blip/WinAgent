...
class MainWindow(QMainWindow):
    ...
    def _rebuild_agent(self) -> None:
        ...
        self.agent = Agent(self.config, self.backend, events=events)
    ...
    def send_message(self, text: str) -> None:
        ...
        if self.config.minimize_gui_while_running and self.backend and self.backend.name == "windows":
            self.showMinimized()
    ...
    def _refresh_info(self) -> None:
        ...
        try:
            ...
            pairs = [(k, json.dumps(v, ensure_ascii=False) if isinstance(v, (dict, list)) else v) for k, v in info.items()]
