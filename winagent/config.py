...
#              or when the agent needs to click/type underneath it, and comes back right away
#   minimize - minimise the main window and show nothing (legacy behaviour)
#   none     - leave the window alone (it may appear in the agent's screenshots)
GUI_MODES = ("overlay", "visible", "minimize", "none")
OVERLAY_CORNERS = ("bottom-right", "bottom-left", "top-right", "top-left")
...
class Config:
    ...
    gui_mode_while_running: str = "overlay"   # overlay | visible | minimize | none
    ...
    def validate(self) -> list[str]:
        ...
        if self.gui_mode_while_running not in GUI_MODES:
            problems.append(f"gui_mode_while_running must be one of {GUI_MODES}.")
        if self.overlay_corner not in OVERLAY_CORNERS:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Config":
        ...
        kwargs: dict[str, Any] = {}
        data = dict(data or {})
        # legacy boolean (< 0.2): minimize_gui_while_running -> gui_mode_while_running
        if "gui_mode_while_running" not in data and "minimize_gui_while_running" in data:
            legacy = _coerce(data.pop("minimize_gui_while_running"), "bool", True)
            data["gui_mode_while_running"] = "overlay" if legacy else "none"
        ...
        cfg = cls(**kwargs)
        cfg.gui_mode_while_running = (cfg.gui_mode_while_running or "overlay").strip().lower()
        cfg.overlay_corner = (cfg.overlay_corner or "bottom-right").strip().lower().replace("_", "-")
        return cfg
