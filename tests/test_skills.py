...
def test_agent_includes_skills_in_system_prompt(config, backend, tmp_path):
    (tmp_path / "pdf-export.md").write_text("### Steps\nExport via the Report menu.\n", encoding="utf-8")
    agent = Agent(config, backend, llm=None, )
    agent.skills = load_skills(extra_dirs=[tmp_path])
    from winagent.skills import skills_section
    agent._skills_section = skills_section(agent.skills)
    sys_msg = agent._system_message()
    assert "### Skill: pdf-export" in sys_msg["content"]
    assert "Installed skills" in sys_msg["content"]


def test_no_skills_adds_nothing(config, backend):
    agent = Agent(config, backend, llm=None)
    agent._skills_section = skills_section([])
    sys_msg = agent._system_message()
